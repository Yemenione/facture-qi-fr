...
export default function CreditNotesPage() {
    ...
    const loadCreditNotes = async () => {
        try {
            const data = await invoiceService.findAll('CREDIT_NOTE')
            setCreditNotes(data)
        } catch (error) {
    ...
    const sendEmail = async (id: string, email?: string) => {
        ...
        if (!confirm(`Envoyer l'avoir par email à ${email} ?`)) return;
    ...
    const deleteCreditNote = async (id: string, number: string) => {
    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto p-2 pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-4xl font-extrabold tracking-tight text-slate-900">Avoirs</h1>
                    <p className="text-slate-500 mt-1">Gestion des notes de crédit et remboursements.</p>
                </div>
                <Button asChild className="bg-slate-900 shadow-lg hover:bg-slate-800 rounded-full">
                    <Link href="/dashboard/credit-notes/new">
                        <Plus className="mr-2 h-4 w-4" /> Créer un Avoir
                    </Link>
                </Button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="bg-white border-slate-100 shadow-sm">
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium text-slate-500">Brouillons</CardTitle>
                ...
                <Card className="bg-white border-slate-100 shadow-sm">
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium text-slate-500">Validés/Remboursés</CardTitle>
                        <FileText className="h-4 w-4 text-green-500" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-3xl font-bold text-slate-900">
                            {creditNotes.filter(cn => cn.status === 'VALIDATED' || cn.status === 'PAID').length}
                        </div>
                    </CardContent>
                </Card>
                <Card className="bg-white border-slate-100 shadow-sm">
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium text-slate-500">Total Avoirs</CardTitle>
                                        ...
                                        <td className="p-4 align-middle">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="font-semibold text-slate-900">{cn.invoiceNumber || 'BROUILLON'}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="p-4 align-middle font-medium text-slate-700">{cn.client?.name}</td>
                                        <td className="p-4 align-middle text-slate-500">{new Date(cn.issueDate).toLocaleDateString('fr-FR')}</td>
                                        <td className="p-4 align-middle font-bold text-slate-900">{formatCurrency(cn.total)}</td>
                                        <td className="p-4 align-middle">
                                            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium
                                                ${cn.status === 'PAID' ? 'bg-green-100 text-green-800' :
                                                    cn.status === 'VALIDATED' ? 'bg-blue-100 text-blue-800' :
                                            ...
                                            <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                {cn.status === 'DRAFT' && (
                                                    <Button variant="ghost" size="icon" asChild className="hover:text-slate-900">
                                                        <Link href={`/dashboard/credit-notes/${cn.id}/edit`}>
                                                            <Pencil className="h-4 w-4" />
                                                        </Link>
                                                    </Button>
                                                )}

                                                <Button variant="ghost" size="icon" onClick={() => sendEmail(cn.id, cn.client?.email)} className="hover:text-indigo-600">
                                                    <Mail className="h-4 w-4" />
                                                </Button>

                                                <Button variant="ghost" size="icon" onClick={() => downloadPdf(cn.id, cn.invoiceNumber)} className="hover:text-slate-900">
                                                    <Download className="h-4 w-4" />
                                                </Button>

                                                <Button variant="ghost" size="icon" onClick={() => deleteCreditNote(cn.id, cn.invoiceNumber)} className="hover:text-red-600">
