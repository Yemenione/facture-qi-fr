...
import { Search, Loader2, Save, Trash } from "lucide-react"
...
export default function EditClientPage() {
    ...
    const handleSiretSearch = async () => {
        ...
        try {
            ...
            if (data) {
                ...
                alert("Données mises à jour depuis l'annuaire !");
            }
        } catch (error) {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert(`Erreur: ${error.response?.data?.message || "Impossible de modifier le client"}`)
        } finally {
            setSaving(false)
        }
    }
    ...
    const onDelete = async () => {
        ...
        try {
            ...
            if (error.response?.status === 500 || error.response?.data?.message?.includes('Constraint')) {
                alert("Impossible de supprimer ce client : il possède des documents (factures/devis) associés. Veuillez d'abord supprimer ses documents.")
            } else {
    ...
    return (
        <div className="max-w-2xl mx-auto py-8 mb-20">
